...
curl -k -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' --header "Authorization: Bearer $TOKEN"  -d @jsons/send_be_main_link_theme_fullscreen.json 'https://bridge-internmatch-staging.outcome-hub.com/api/service?channel=webdata'
