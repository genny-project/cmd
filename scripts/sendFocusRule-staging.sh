...
curl -k -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' --header "Authorization: Bearer $TOKEN" -d '{  "msg_type":"EVT_MSG", "event_type":"FOCUS_RULE_GROUP", "data":{ "code":"'${rulegroup}'" }   }' 'https://bridge.genny.life/api/service'
